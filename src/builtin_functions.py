...
class BuiltInFunction:
    ...
    def validate_params(self, actual_params):
        ...
        if len(actual_params) != self.param_count:
            return (False, f"Function {self.name} expects {self.param_count} parameter(s), but got {len(actual_params)}")
        ...
        for i, (param_value, param_type) in enumerate(actual_params):
            ...
            if param_type != expected_type and expected_type != 'ANY':
                return (False, f"Parameter {i+1} of {self.name} should be {expected_type}, but got {param_type}")
