        ...
        if self.current_char == quote_char:
            ...
            if quote_char == "'" and len(result) == 1:
                ...
                raise SyntaxError(
                    ...
                    start_col,
                    (
                        "Use double quotes for strings or ensure single quotes "
        ...
        while self.current_char is not None:
            ...
            if self.current_char == "<":
                if self.peek() == "-":
                    ...
                    self.advance()
                    tokens.append(
                        Token(TokenType.ASSIGN, "<-", self.line, self.column - 2)
                    )
                elif self.peek() == ">":
                    self.advance()
                    self.advance()
                    tokens.append(
                        Token(TokenType.NEQ, "<>", self.line, self.column - 2)
                    )
                elif self.peek() == "=":
                    self.advance()
                    self.advance()
                    tokens.append(
                        Token(TokenType.LTE, "<=", self.line, self.column - 2)
                    )
                else:
                    self.advance()
            ...
            if self.current_char == ">":
                if self.peek() == "=":
                    ...
                    self.advance()
                    tokens.append(
                        Token(TokenType.GTE, ">=", self.line, self.column - 2)
                    )
                else:
                    self.advance()
            ...
            if self.current_char == "*":
                self.advance()
                tokens.append(
                    Token(TokenType.MULTIPLY, "*", self.line, self.column - 1)
                )
                continue
            ...
            if self.current_char == "[":
                self.advance()
                tokens.append(
                    Token(TokenType.LBRACKET, "[", self.line, self.column - 1)
                )
                continue
            ...
            if self.current_char == "]":
                self.advance()
                tokens.append(
                    Token(TokenType.RBRACKET, "]", self.line, self.column - 1)
                )
                continue
