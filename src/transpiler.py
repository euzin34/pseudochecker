...
class Transpiler:
    ...
    def visit_Declaration(self, node):
        if node.is_array:
            bounds_str = ", ".join(
                [
                    f"{self.transpile(lower)}:{self.transpile(upper)}"
                    for lower, upper in node.array_bounds
                ]
            )
            return (
                f"{self.indent()}# DECLARE {node.identifier} : ARRAY[{bounds_str}] "
                f"OF {node.dtype}"
            )
    ...
    def visit_ConstantDeclaration(self, node):
        return (
            f"{self.indent()}{node.identifier} = "
            f"{self.transpile(node.value)}  # CONSTANT"
        )
    ...
    def visit_Assignment(self, node):
        if node.index:
            ...
            return (
                f"{self.indent()}{node.identifier}[{indices}] = "
                f"{self.transpile(node.expression)}"
            )
    ...
    def visit_FunctionDeclaration(self, node):
        params = ", ".join([name for name, _, _ in node.parameters])
        result = (
            f"{self.indent()}def {node.name}({params}):  # RETURNS {node.return_type}\n"
        )
        self.indent_level += 1
        if node.body:
            for stmt in node.body:
