...
class SemanticAnalyzer:
    ...
    def visit_Declaration(self, node):
        ...
        if self.current_scope.lookup_local(node.identifier):
            self.errors.add_error(
                SemanticError(
                    ...
                    1,
                    1,  # We don't have line info in AST nodes yet
        ...
        # Validate data type
        if (
            node.dtype.upper() not in VALID_DATA_TYPES
            and node.dtype not in self.user_types
        ):
            self.errors.add_error(
                TypeError(
            ...
            if symbol.data_type and expr_type:
                if not self.types_compatible(symbol.data_type, expr_type):
                    self.errors.add_error(
                        TypeError(
                            (
                                f"Type mismatch: Cannot assign {expr_type} to array of "
                                f"{symbol.data_type}"
                            ),
                            1,
                            1,
    ...
    def visit_FunctionDeclaration(self, node):
        ...
        if not self.has_return_statement:
            self.errors.add_error(
                SemanticError(
                    (
                        f"Function '{node.name}' must return a value of type "
                        f"{node.return_type}"
                    ),
                    1,
                    1,
                    "Add 'RETURN <expression>' before the end of the function",
                )
            )
                ...
                if func_symbol and func_symbol.return_type:
                    if not self.types_compatible(func_symbol.return_type, expr_type):
                        msg = (
                            ...
                            f"Function expects {func_symbol.return_type} "
            ...
            self.errors.add_error(
                SemanticError(
                    (
                        ...
                        f"arguments but got {len(node.arguments)}"
                    ),
                    1,
                    1,
    ...
    def visit_BuiltInFunctionCall(self, node):
        ...
        # Validate call
        is_valid, error_msg, return_type = self.builtin_registry.validate_call(
            node.name, arg_types
        )
        if not is_valid:
            self.errors.add_error(
    ...
    def visit_ProcedureCall(self, node):
        ...
        if len(node.arguments) != len(symbol.parameters):
            self.errors.add_error(
                SemanticError(
                    (
                        f"Procedure '{node.name}' expects {len(symbol.parameters)} "
                        f"arguments but got {len(node.arguments)}"
                    ),
                    1,
                    1,
